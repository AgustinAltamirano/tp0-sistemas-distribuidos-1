...
En este ejercicio se extiende el protocolo con nuevos mensajes para finalizar las apuestas y consultar los ganadores.
Además, el servidor pasa a ser concurrente, atendiendo a múltiples agencias en paralelo con mecanismos de
sincronización.

#### Nuevos mensajes del protocolo

##### FinalizeBets (código de mensaje: 3)

Enviado por el cliente al servidor para indicar que la agencia terminó de enviar todas sus apuestas.

| Campo       | Tamaño  | Tipo   | Descripción                                |
| ----------- | ------- | ------ | ------------------------------------------ |
| messageCode | 2 bytes | uint16 | Código del mensaje (`3` = FINALIZE_BETS)   |
| agencyId    | 2 bytes | uint16 | Id de la agencia que finaliza sus apuestas |

##### AskWinnersList (código de mensaje: 4)

Enviado por el cliente al servidor para solicitar la lista de ganadores de su agencia.

| Campo       | Tamaño  | Tipo   | Descripción                                 |
| ----------- | ------- | ------ | ------------------------------------------- |
| messageCode | 2 bytes | uint16 | Código del mensaje (`4` = ASK_WINNERS_LIST) |
| agencyId    | 2 bytes | uint16 | Id de la agencia que consulta sus ganadores |

##### GiveWinnersList (código de mensaje: 5)

Enviado por el servidor al cliente como respuesta a un `AskWinnersList`, con los documentos de los ganadores.

**Header:**

| Campo         | Tamaño  | Tipo   | Descripción                                  |
| ------------- | ------- | ------ | -------------------------------------------- |
| messageCode   | 2 bytes | uint16 | Código del mensaje (`5` = GIVE_WINNERS_LIST) |
| agencyId      | 2 bytes | uint16 | Id de la agencia                             |
| winnersAmount | 4 bytes | uint32 | Cantidad de ganadores                        |

**Por cada ganador** (repetido `winnersAmount` veces):

| Campo    | Tamaño  | Tipo   | Descripción     |
| -------- | ------- | ------ | --------------- |
| document | 4 bytes | uint32 | DNI del ganador |

#### Flujo de finalización y consulta de ganadores

Una vez que la agencia terminó de enviar todos sus batches de apuestas, el flujo continúa de la siguiente manera:

1. El cliente envía un mensaje **FinalizeBets** al servidor, indicando que no tiene más apuestas por registrar.
2. El cliente envía un mensaje **AskWinnersList** para solicitar los ganadores de su agencia.
3. En el servidor, al recibir `FinalizeBets`, el hilo de la agencia se bloquea en el `LotteryEventMonitor` hasta que
   **todas** las agencias hayan finalizado sus apuestas. Recién cuando la última agencia finaliza, se realiza el sorteo
   y todos los hilos se desbloquean.
4. El servidor carga todas las apuestas almacenadas, filtra las que pertenecen a la agencia solicitante y cuyo número
   coincide con el número ganador (`7574`), y responde con un mensaje **GiveWinnersList** conteniendo los DNIs de los
   ganadores.
5. El cliente recibe la lista de ganadores y loguea el resultado.

#### Servidor concurrente

El servidor ahora atiende a múltiples clientes de forma concurrente. Por cada conexión aceptada, se crea un
**thread** dedicado que ejecuta toda la lógica de la lotería central para esa agencia:
...
Al solicitar el shutdown, el servidor espera a que todos los threads terminen mediante `join()` antes de cerrar.

#### Mecanismos de sincronización

Se utilizan dos monitores para coordinar el acceso concurrente:

1. **BetsStorageMonitor**: protege el acceso al archivo `bets.csv` con un `threading.Lock()`. Cada escritura de un
   batch de apuestas se realiza dentro del lock, evitando que dos hilos escriban simultáneamente y corrompan el
   archivo.

2. **LotteryEventMonitor**: implementa una barrera de sincronización usando un `threading.Lock()` y un
   `threading.Event()`. Mantiene un contador inicializado con la cantidad de agencias esperadas. Cada vez que una
   agencia llama a `wait()`, el contador se decrementa bajo lock. Cuando llega a cero (la última agencia finalizó),
   se señaliza el `Event`, desbloqueando a todos los hilos que estaban esperando. De esta forma, ninguna agencia
   puede consultar los ganadores hasta que todas hayan terminado de registrar sus apuestas.
